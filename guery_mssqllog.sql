/* This is example how to query datetime of data changes, using sql server transaction log, if audit routine wasn't set*/
...
left join sys.fn_dblog(NULL,NULL) l2 on l2.[Transaction ID]=l1.[Transaction ID]  
  )
SELECT PAGE,[Transaction ID], min([Begin Time]),min([End Time]),min(l1_uname),min(l2_tname)
FROM tlog
